...
List Cluster_Integrated_Sampler(
    ...
    int list_offset = 12;
    int list_length = (1 + list_offset + number_of_outer_itterations + 6*number_of_MH_itterations);
    ...
    for(int n = 0; n < number_of_outer_itterations; ++n){
        ...
        for(int i = 0; i < number_of_Gibbs_itterations; ++i){
            ...
            // ===================== Hard Cluster Topics ====================== //
                ...
                for(int t = 0; t < number_of_topics; ++t){
                    ...
                    NumericVector cluster_edges_assigned(number_of_clusters);
                    ...
                    for(int x = 0; x < number_of_clusters; ++x){
                        //double temp = cluster_probabilities[x];
                        //cluster_probabilities[x] = temp/(cluster_edges_assigned[x] +1);
                        //cluster_probabilities[x] += smallest/1000;
                        //cluster_probabilities[x] += 0.00000000001;
                    }
        ...
        for(int d = 0; d < number_of_documents; ++d){
            ...
            for(int w = 0; w < number_of_tokens; ++w){
                ...
                int topic_index = token_topic_assignments1[w] -1;
                ...
                double normalized_value = log(topic_distribution[topic_index]/sum(topic_distribution));
                Current_Alpha_Corpus_Likelihood += normalized_value;
        ...
        for(int i = 0; i < number_of_MH_itterations; ++i){
            ...
            if(n == (number_of_outer_itterations -1)){
                ...
                to_return[list_offset + number_of_outer_itterations + MH_Counter] = ints;
                to_return[list_offset + number_of_outer_itterations + number_of_MH_itterations + MH_Counter] = lat_pos;
                to_return[list_offset + number_of_outer_itterations + 2*number_of_MH_itterations + MH_Counter] = bets;
                to_return[list_offset + number_of_outer_itterations + 3*number_of_MH_itterations + MH_Counter] = cluster_accepted;
                to_return[list_offset + number_of_outer_itterations + 4*number_of_MH_itterations + MH_Counter] = Cur_Proposed_MH_Likelihoods;
                to_return[list_offset + number_of_outer_itterations + 5*number_of_MH_itterations + MH_Counter] = Cur_Current_MH_Likelihoods;
                MH_Counter += 1;
            }
